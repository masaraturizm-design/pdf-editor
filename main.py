...
async def upload_pdf(file: UploadFile = File(...)):
    ...
    pid = uuid.uuid4().hex
    with open(UPLOAD_DIR/f"{pid}.pdf","wb") as f: shutil.copyfileobj(file.file,f)
...
async def upload_image(file: UploadFile = File(...)):
    ...
    iid = uuid.uuid4().hex
    with open(UPLOAD_DIR/f"{iid}{ext}","wb") as f: shutil.copyfileobj(file.file,f)
...
async def process_command(req: CommandRequest):
    ...
    if not actions: raise HTTPException(422,"Command not understood")
    rid = uuid.uuid4().hex
    rp = RESULT_DIR/f"{rid}.pdf"
    if not pdf_proc.apply_actions(str(pdf_path), actions, str(rp)) or not rp.exists():
        raise HTTPException(500,"PDF could not be edited")
...
async def download_pdf(result_id: str):
    ...
    return FileResponse(str(p), media_type="application/pdf", filename=f"edited_{result_id[:8]}.pdf")
